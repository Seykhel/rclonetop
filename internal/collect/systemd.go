...
//
// The reading itself belongs to execstart; what is kept here is the cache,
// because working the answer out costs a file read and a unit's command line
// does not change between ticks.
func (s *Systemd) logFileForUnit(scope, name, execStart string) string {
	...
	found := execstart.LogFile(execStart, home)
...
func (s *Systemd) classify(ctx context.Context, scope string, services []string) map[string]bool {
	...
	for _, name := range undecided {
		hit := execstart.DrivesRclone(execs[name]["ExecStart"])
